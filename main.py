...
class RevisedMeanFunction(gpflow.functions.MeanFunction):
    ...
    def __init__(self, function_data=None):
        super().__init__()
        self.current_model = function_data

    def __call__(self, x):
        if self.current_model is None:
            return tf.zeros_like(x, dtype=tf.float64)
        mean_values, _ = self.current_model.predict_f(x)
        return mean_values
...
def run_walkforward_analysis(data):
    ...
    current_model = gpflow.models.GPR(
        ...
        mean_function=RevisedMeanFunction()
    )
    ...
    for i in range(len(data)):
        ...
        prediction_window = tf.convert_to_tensor(np.arange((10*i)+50, (10*i)+60).reshape(-1, 1), dtype=tf.float64)
        ...
        walk_forward_results.update_predictions(f_mean.numpy().flatten(), f_var.numpy().flatten())
        walk_forward_results.update_loss_profile(data.test_data[i], f_mean.numpy().flatten())

        # change mean function for next iteration
        current_model.mean_function.function_data = current_model
